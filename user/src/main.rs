...
fn init() {
    unsafe {
        ...
        for i in 0..4 {
            perf_fds[i] = libc::syscall(libc::SYS_perf_event_open, &_attr, i, 0, -1, 0) as libc::c_int;
            bpf_map_update_elem(map_fd[0], &i as *const _ as *const _, &perf_fds[i] as *const _ as *const _, BPF_ANY as u64);
...
fn main() {
    unsafe {
        ...
        for i in 0..4 {
            perf_event_mmap_header(perf_fds[i], &mut headers[i]);
        }
